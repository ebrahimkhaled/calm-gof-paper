...
one_rep <- function(r, dep, gamma) {
  ...
  # analytic: first-order reference (R0) and the observable-variance references (R2, R4) on the same fit
  pc0 <- cf_pieces(X, y, lambda, G = G); rf <- cf_references(X, y, lambda, pc0)
  a  <- list(SC.HL = list(p.value = cf_pvalue(pc0$S_dec, pc0$Om_MLE)), SC.EDGE = list(p.value = cf_pvalue(pc0$S_edge, pc0$Om_MLE, pc0$Z)))
  a2 <- list(SC.HL = list(p.value = cf_pvalue(pc0$S_dec, rf$R2)),      SC.EDGE = list(p.value = cf_pvalue(pc0$S_edge, rf$R2, pc0$Z)))
  a4 <- list(SC.HL = list(p.value = cf_pvalue(pc0$S_dec, rf$R4)),      SC.EDGE = list(p.value = cf_pvalue(pc0$S_edge, rf$R4, pc0$Z)))
  ...
  data.frame(rep = r, dep = dep, gamma = gamma,
             ...
             r2_hl = a2$SC.HL$p.value, r2_edge = a2$SC.EDGE$p.value,
             r4_hl = a4$SC.HL$p.value, r4_edge = a4$SC.EDGE$p.value,
...
write.csv(res, file.path(OUT, "E2a2_power_perrep.csv"), row.names = FALSE)

rej <- function(p) mean(p < 0.05)
agg <- aggregate(cbind(an_hl, an_edge, r2_hl, r2_edge, r4_hl, r4_edge, pp_hl, pp_edge, cl_hl, cl_edge) ~ dep + gamma, data = res, FUN = rej)
# paired differences with SE (over the 0/1 rejection indicators)
pd <- aggregate(cbind(d_hl = (r4_hl < 0.05) - (pp_hl < 0.05), d_edge = (r4_edge < 0.05) - (pp_edge < 0.05)) ~ dep + gamma, data = res,
...
write.csv(agg, file.path(OUT, "E2a2_power_summary.csv"), row.names = FALSE)

options(width = 200)
cat(sprintf("\n=== E2a | design A | R = %d | prepivot B = %d | %.1f min ===\n", R, B_BOOT, as.numeric(difftime(Sys.time(), t0, units = "mins"))))
show <- agg; for (v in c("an_hl", "an_edge", "r2_hl", "r2_edge", "r4_hl", "r4_edge", "pp_hl", "pp_edge", "cl_hl", "cl_edge")) show[[v]] <- sprintf("%.3f", show[[v]])
show$diff_hl <- sprintf("%+.3f (%.3f)", agg$diff_hl, agg$se_hl); show$diff_edge <- sprintf("%+.3f (%.3f)", agg$diff_edge, agg$se_edge)
print(show[, c("dep", "gamma", "an_hl", "r2_hl", "r4_hl", "pp_hl", "cl_hl", "diff_hl", "an_edge", "r2_edge", "r4_edge", "pp_edge", "cl_edge", "diff_edge")], row.names = FALSE)
